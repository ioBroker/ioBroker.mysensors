var presentation = {
    ...
    'S_ARDUINO_NODE':       {index: 17, role: '',                  vars: ['I_BATTERY_LEVEL', 'I_VERSION', 'I_SKETCH_NAME', 'I_CONFIG']},
...
function getMetaInfo(packet, ip, port, config) {
    ...
    if (config.name) {
        result.push({
            _id:            (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id,
            common: {
...
function getMetaInfo2(packet, ip, port, config, subType, common_name) {
    ...
    var id = (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + '_' + subType.replace('S_', '') + /*'.' + packet.subType + */ '.' + varType;
