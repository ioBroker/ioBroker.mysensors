const presentation = {
    'S_DOOR':               {index: 0,  role: 'door',              vars: ['V_TRIPPED', 'V_ARMED']},
    'S_MOTION':             {index: 1,  role: 'motion',            vars: ['V_TRIPPED', 'V_ARMED']},
    'S_SMOKE':              {index: 2,  role: 'alarm.smoke',       vars: ['V_TRIPPED', 'V_ARMED']},
    'S_BINARY':             {index: 3,  role: 'relay',             vars: ['V_STATUS', 'V_WATT']},
    ...
    'S_TEMP':               {index: 6,  role: 'temperature',       vars: ['V_TEMP','V_ID']},
    'S_HUM':                {index: 7,  role: 'humidity',          vars: ['V_HUM']},
    'S_BARO':               {index: 8,  role: 'pressure',          vars: ['V_PRESSURE','V_FORECAST']},
    ...
    'S_POWER':              {index: 13, role: 'power.consumption', vars: ['V_WATT', 'V_KWH','V_VAR', 'V_VA', 'V_POWER_FACTOR']},
    'S_HEATER':             {index: 14, role: 'temperature',       vars: ['V_HVAC_SETPOINT_HEAT', 'V_HVAC_FLOW_STATE', 'V_TEMP']},
    'S_DISTANCE':           {index: 15, role: 'distance',          vars: ['V_DISTANCE']},
    'S_LIGHT_LEVEL':        {index: 16, role: 'brightness',        vars: ['V_LIGHT_LEVEL', 'V_LEVEL']},
    'S_ARDUINO_NODE':       {index: 17, role: '',                  vars: ['I_BATTERY_LEVEL', 'I_SKETCH_VERSION', 'I_VERSION', 'I_SKETCH_NAME', 'I_CONFIG','I_HEARTBEAT_RESPONSE','I_PRE_SLEEP_NOTIFICATION','I_POST_SLEEP_NOTIFICATION']},
    ...
    'S_AIR_QUALITY':        {index: 22, role: '',                  vars: ['V_LEVEL']},
    'S_CUSTOM':             {index: 23, role: '',                  vars: ['V_VAR1', 'V_VAR2', 'V_VAR3', 'V_VAR4', 'V_VAR5']},
    'S_DUST':               {index: 24, role: 'alarm.dust',        vars: ['V_LEVEL']},
    ...
    'S_HVAC':               {index: 29, role: 'temperature',       vars: ['V_HVAC_SETPOINT_HEAT', 'V_HVAC_SETPOINT_COLD', 'V_HVAC_FLOW_STATE', 'V_HVAC_FLOW_MODE', 'V_TEMP']},
    ...
    'S_WATER_LEAK':         {index: 32, role: 'alarm.water',       vars: ['V_TRIPPED', 'V_ARMED']},
...
const vars = {
    ...
    'V_PRESSURE':           {index: 4,  role: 'value',     type: 'number',     read: true, write: false, def: 0, min: 0, max: 1200, unit: 'hPa'},     //Atmospheric Pressure
    ...
    'V_SCENE_ON':           {index: 19, role: 'button',    type: 'number',    read: true, write: true , def: false},     //Turn on a scene
    'V_SCENE_OFF':          {index: 20, role: 'button',    type: 'number',    read: true, write: true , def: false},     //Turn of a scene
    ...
    'V_VAR':                {index: 54, role: 'value',     type: 'number',     read: true, write: false},             //S_POWER, Reactive power: volt-ampere reactive (const)
    ...
    'I_HEARTBEAT_RESPONSE': {index: 22, role: 'state',     type: 'number',     read: true, write: false, def: 0, min: 0, unit: 'ms'}, // Heartbeat response
    'I_PRE_SLEEP_NOTIFICATION': {index: 32, role: 'state',     type: 'number',     read: true, write: false, def: 0, min: 0, unit: 'ms'},      // Message sent before node is going to sleep
    'I_POST_SLEEP_NOTIFICATION': {index: 33, role: 'state',     type: 'number',     read: true, write: false, def: 0, min: 0, unit: 'ms'},     // Message sent after node woke up (if enabled)
...
function getPresentationEntry(subType) {
    const type = presentation[subType];
    if (!type) {
        const st = parseInt(subType);
        for (const n in presentation) {
...
function getMetaInfo(packet, ip, port, config) {
    config = config || {};
    const type = getPresentationEntry(packet.subType);
    ...
    for (let v = 0; v < type.vars.length; v++) {
        const variable = vars[type.vars[v]];
        const id = _getName(ip, packet) + '.' + type.vars[v];
        if (variable) {
            result.push({
                _id:            id,
                common: {
    ...
    if (config.metric === 'Imperial') {
        for (let i = 0; i < result.length; i++) {
            if (result[i].common.unit === '°C') {
                result[i].common.unit = '°F';
            }
        }
    }
...
function getMetaInfo2(packet, ip, port, config, subType, common_name) {
    ...
    const varType = packet.subType;
    const variable = vars[varType];
    const id = _getName(ip, packet, subType) + '.' + varType;
