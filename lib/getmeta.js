...
function getMetaInfo(packet, ip, port, config) {
    ...
    var result = [
        {
            _id:            (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + (packet.subType ? '.' + packet.subType : ''),
            common: {
                name:       packet.payload || ((ip ? ip.replace(/\./g, '.') + '_'  : '') + packet.id + '.' + packet.childId + '.' + (packet.subType ? '.' + packet.subType : '')),
                role:       type.role
            },
            native: {
                ip:         ip,
    ...
    for (var v = 0; v < type.vars.length; v++) {
        ...
        var id = (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + '.' + packet.subType + '.' + type.vars[v];
        result.push({
            _id:            (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + '.' + packet.subType + '.' + type.vars[v],
            common: {
    ...
    // add battery object
    result.push({
        _id:            (ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + '.S_BATTERY',
        common: {
            name:       packet.payload || ((ip ? ip.replace(/\./g, '_') + '.'  : '') + packet.id + '.' + packet.childId + '.S_BATTERY'),
            type:       'number',
            role:       'value.battery',
            min:        0,
            max:        100,
