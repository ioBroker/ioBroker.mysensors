...
function MySensors(options, log) {
    ...
    if (options.type === 'udp') {
        ...
        this._interface.on('message', function (msg, rinfo) {
            if (data.split(';').length < 6) {
                ...
                this.emit('data', rinfo.address + msg);
            }
        });

        this._interface.on('listening', function (msg, rinfo) {
            var address = this._interface.address();
            console.log('server listening ${address.address}:${address.port}');
        }.bind(this));

        this._interface.bind(options.port || 8099);
    ...
    this.write = function (data) {
        if (this._interface) {
            adapter.log.debug('Send     raw data: ' + data);
