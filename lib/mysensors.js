...
function MySensors(options, log, onCreated) {
	...
	function openSerial() {
        ...
        try {
            ...
            ReadlineParser = ReadlineParser || require('@serialport/parser-readline').ReadlineParser;
        } catch (e) {
        ...
        if (SerialPort) {
            const portConfig = {
                ...
                autoOpen: false,
                path:     options.comName
            };

            if (options.comName) {
                try {
                    that._interface = new SerialPort(portConfig/*, false*/);
                    that._interfaceParser = that._interface.pipe(new ReadlineParser());
